...
public class Main {
    ...
    public static void main(String[] args) throws FileNotFoundException {
        ...
        Graph g = new Graph("./facebook_combined.edges");

        g.checkValidity();

        Set<Node> independent = getIndependentSet(g);

        System.out.println("Check if set is independent:");
        if (isIndependent(independent)){
            System.out.println("Independent!");
        }else{
            System.out.println("Unfortunately, the set of nodes is not an independent set");
        }
        System.out.println("\nNumber of elements:");
